...
export const api = {
  ...
  async getCarbonFootprintAnalysis(): Promise<any> {
    await new Promise(resolve => setTimeout(resolve, 800));
    
    // Check if we have user uploaded data
    const hasUploadedData = localStorage.getItem('energySage_userHasUploadedData') === 'true';
    
    if (!hasUploadedData || uploadedData.length === 0) {
      console.log('📊 No uploaded data - carbon analysis unavailable');
      return null;
    }

    // Calculate carbon footprint from real uploaded data
    const recentData = uploadedData.slice(-168); // Last 7 days
    const totalKwh = recentData.reduce((sum, reading) => sum + reading.kwh, 0);
    
    // US average carbon intensity: ~0.85 kg CO2/kWh
    const carbonIntensity = 0.85;
    const dailyCO2 = (totalKwh / recentData.length) * 24 * carbonIntensity;
    const monthlyCO2 = dailyCO2 * 30;
    const yearlyProjection = dailyCO2 * 365;
    
    // Calculate current footprint in tons per year
    const currentFootprint = yearlyProjection / 1000;
    
    console.log(`📊 Carbon analysis - Total kWh: ${totalKwh.toFixed(2)}, Daily CO2: ${dailyCO2.toFixed(2)} kg`);

    return {
      current_footprint: Math.round(currentFootprint * 10) / 10,
      daily_co2: Math.round(dailyCO2 * 10) / 10,
      monthly_co2: Math.round(monthlyCO2),
      yearly_projection: Math.round(yearlyProjection),
      carbon_intensity: carbonIntensity,
      sustainability_score: Math.min(100, Math.max(0, 100 - (currentFootprint * 15))),
      grade: currentFootprint < 3 ? 'A+' : currentFootprint < 4 ? 'A' : currentFootprint < 5 ? 'B+' : currentFootprint < 6 ? 'B' : 'C',
      peer_comparison: {
        percentile: Math.max(10, 90 - (currentFootprint * 10)),
        average_footprint: 6.1
      },
      real_time_data: {
        current_consumption: Math.round((recentData[recentData.length - 1]?.kwh || 2.8) * 100) / 100,
        co2_rate: Math.round(dailyCO2 / 24 * 100) / 100,
        today_savings: Math.round((6.1 - currentFootprint) * 365 / 365 * 100) / 100,
        efficiency_score: Math.min(100, Math.max(20, 100 - (currentFootprint * 12)))
      },
      offset_recommendations: [
        {
          ...
          co2_offset: Math.round(currentFootprint * 0.3 * 100) / 100,
          cost_estimate: 25,
        ...
        },
        {
          ...
          co2_offset: Math.round(currentFootprint * 0.7 * 100) / 100,
          cost_estimate: 150,
        ...
        },
        {
          ...
          co2_offset: Math.round(currentFootprint * 0.15 * 100) / 100,
          cost_estimate: 85,
      ...
      reduction_tips: [
        {
          category: 'heating',
          tip: `Optimize ${recentData.filter(r => r.device.toLowerCase().includes('heat')).length > 0 ? 'your heating system' : 'heating settings'} during peak hours`,
          potential_reduction: Math.round(currentFootprint * 0.2 * 100) / 100,
          difficulty: 'easy',
          cost: 'free',
          impact_score: 9,
        ...
        {
          category: 'appliances',
          tip: `Use smart power strips for ${[...new Set(recentData.map(r => r.device))].slice(0, 3).join(', ')} to eliminate phantom loads`,
          potential_reduction: Math.round(currentFootprint * 0.1 * 100) / 100,
          difficulty: 'easy',
          cost: 'low',
          impact_score: 8,
        ...
        },
        {
          ...
          potential_reduction: Math.round(currentFootprint * 0.3 * 100) / 100,
          difficulty: 'moderate',
          cost: 'medium',
          impact_score: 9,
        ...
        },
        {
          ...
          potential_reduction: Math.round(currentFootprint * 0.08 * 100) / 100,
          difficulty: 'easy',
          cost: 'low',
          impact_score: 7,
        ...
        {
          category: 'behavior',
          tip: `Shift ${recentData.filter(r => r.device.toLowerCase().includes('wash')).length > 0 ? 'laundry and dishwasher' : 'high-consumption appliances'} to off-peak hours`,
          potential_reduction: Math.round(currentFootprint * 0.15 * 100) / 100,
          difficulty: 'easy',
          cost: 'free',
          impact_score: 8,
      ...
      achievements: [
        {
          ...
          icon: '🌱',
          unlocked: currentFootprint < 5,
          progress: Math.min(100, Math.max(0, (6.1 - currentFootprint) / 6.1 * 100)),
          target: 100,
          points: 500,
          unlock_date: currentFootprint < 5 ? new Date().toISOString().split('T')[0] : undefined
        },
        {
          id: 'carbon_neutral',
        ...
        },
        {
          ...
          icon: '⚡',
          unlocked: false,
          progress: Math.min(100, Math.max(0, 100 - (currentFootprint * 12))),
          target: 100,
          points: 750
        }
      ]
